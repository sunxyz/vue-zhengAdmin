...
/**	// 切换系统
	$('.switch-systems').click(function () {
		...
		$.cookie('bloom-upms-systemid', systemid);
		$.cookie('bloom-upms-systemname', systemname);
		$.cookie('bloom-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('bloom-upms-systemid') || 1;
	var systemname = $.cookie('bloom-upms-systemname') || 'bloom-upms-server';
	var systemtitle = $.cookie('bloom-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
